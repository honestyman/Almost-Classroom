        ...
        <form action="{{ route('group.post.destroy', [$post->group->id, $post->id]) }}" method="POST">
            ...
            <div class="my-3">
                <select name="type" id="type"
                    ...
                    <option value="1" {{ $post->type === 1 ? 'selected' : '' }}>Pouze odkliknutí 'Splněno'
                    </option>
                    <option value="2" {{ $post->type === 2 ? 'selected' : '' }}>'Splněno' doplněno
                        textovým polem
                    </option>
                </select>
            </div>
            ...
            @if ($showDiv)
                <input type="datetime-local" id="deadline" name="deadline" disabled
